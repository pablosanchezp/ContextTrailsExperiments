...
#Path of the jar file (src baselines classic)
...
allKFactorizerRankSys="100 50 10"
...
#NOTE: THERE ARE SOME POIS WITH COORDINATES -1 AND -1, THAT MEANS THAT WE DO NOT HAVE INFORMATION FOR THEM.
# WE WORK AS -1,-1 WERE COORDINATES, BECAUSE THE DISTANCE TO EVERY POI THAT IS -1,-1 WOULD BE HIGHER THAN ANY OTHER POI IN THE CITY
...
do
  ...
  trainFileNoDec=$path_inputs/${city}"_mapped_trails_weather_aggregated_Temp80trainNODEC.csv"
  awk -F',' 'BEGIN {OFS="\t"} {sub(/\..*/, "", $NF); print}' $trainFile > $trainFileNoDec

  testfileNoDec=$path_inputs/${city}"_mapped_trails_weather_aggregated_Temp20testNODEC.csv"
  awk -F',' 'BEGIN {OFS="\t"} {sub(/\..*/, "", $NF); print}' $testfile > $testfileNoDec


  outputRecfile=$recommendationFolder/"$recPrefix"_"$city"_RSys_"SkylineTestOrder".txt
  $javaCommand $jvmMemory -jar $JAR -o skylineRecommenders -trf $trainFileNoDec -tsf $testfileNoDec -cIndex true -rr "SkylineTestOrder" -rs "notUsed" -nI $itemsRecommended -n 20 -orf $outputRecfile
  ...
  do
    ...
    $javaCommand $jvmMemory -jar $JAR -o ranksysOnlyComplete -trf $trainFile -tsf $testfile -cIndex true -rr "$ranksysRecommender" -rs "notUsed" -nI 500 -n 20 -orf $outputRecfile
  ...
  UBsimilarity="SJUS"
  neighbours="100"
  outputRecfile=$recommendationFolder/"$recPrefix"_"$city"_RSys_POI_"$poiRecommender"_UBSim_"$UBsimilarity"_k"$neighbours"_WrongCoordsByMidpoint500Items.txt
  $javaCommand $jvmMemory -jar $JAR -o ranksysOnlyComplete -trf $trainFile -tsf $testfile -cIndex true -rr $poiRecommender -rs $UBsimilarity -nI 500 -n $neighbours -orf $outputRecfile -coordFile $cityCompletePOICoords
      ...
      for bias_reg in $BPRBiasReg
        do
          ...
          do
            ...
            do
              outputRecfile=$recommendationFolder/"$recPrefix"_"$city"_RSys_GeoBPRMF_nF"$factor"_nIt"$BPRNumIter"_LR"$BPRLearnRate"_BR"$bias_reg"_RU"$regU"_RI"$regU"_MaxD"$maxDist"_Rep"$repetition"_WrongCoordsByMidpoint.txt
...
# RankGeoTest
  ...
  do
    ...
    do
      for neighbours in $allneighbours
      do
        outputRecfile=$recommendationFolder/"$recPrefix"_"$city"_RSys_POI_"$poiRecommender"_UBSim_"$UBsimilarity"_k"$neighbours"_WrongCoordsByMidpoint.txt
    ...
    do
      ...
      do
        ...
        do
          ...
          outputRecfile=$pathDest/$recommendationFolder/"$recPrefix"_"$city"_IRENMF_"$configureFileToSelect"_WrongCoordsByMidpoint.txt
